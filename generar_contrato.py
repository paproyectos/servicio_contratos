...
# Directorio base del archivo (para encontrar plantilla.docx de forma segura)
...
def validar(datos: Dict[str, Any], doc: Document):
    ...
    if pares_no_vacios != num:
        raise ValueError(
            f"numero_cuotas={num} no coincide con pares no vacíos={pares_no_vacios}."
        )
    ...
    monto_total = int(
        re.sub(r"[^\d]", "", str(datos.get("monto", "0")) or "0")
    )
    if total != monto_total:
        raise ValueError(
            f"La suma de cuotas ({total}) difiere del monto total ({monto_total})."
        )
...
def expandir_repeat_items(doc: Document, items: list[dict[str, str]]):
    ...
    OPEN_RX = re.compile(
        ...
        r"|\<\<\s*repeat\s*:\s*(?:items|ítems)\s*\>\>)\s*$",
        re.IGNORECASE,
    )
    ...
    CLOSE_RX = re.compile(
        ...
        r"|\{\{\s*/\s*repeat\s*:\s*(?:items|ítems)\s*\}\}"
        r"|\<\<\s*/\s*repeat\s*:\s*(?:items|ítems)\s*\>\>)\s*$",
        re.IGNORECASE,
    )
    ...
    for it in items or []:
        ...
        replace_item_placeholder_in_clone(
            clone, str(it.get("item_descripcion", "") or "")
        )
...
def generar_contrato(
    datos_or_path: Union[Dict[str, Any], str],
    plantilla_path: str,
    salida_dir: str,
    ...
    """
    datos_or_path:
      - dict con los datos del contrato (lo que te manda Airtable), o
      - ruta a archivo (.json/.csv/.xlsx) para modo CLI.
    """

    # 1) Datos desde dict o archivo
    if isinstance(datos_or_path, dict):
        datos = datos_or_path
    else:
        datos = cargar_datos(datos_or_path)

    # 2) Resolver ruta completa de la plantilla
    ...
    # 4) Construir items si vinieron como item_descripcion_1..N
    items = datos.get("items", [])
    if (not items) and any(k.startswith("item_descripcion_") for k in datos.keys()):
        ordenados = sorted(
            [
                (int(re.sub(r"\D", "", k) or 0), v)
                for k, v in datos.items()
                if k.startswith("item_descripcion_")
            ],
            key=lambda x: x[0],
        )
        items = [{"item_descripcion": v} for _, v in ordenados]

    # 5) Validar vs plantilla
    validar(datos, doc)

    # 6) Mapping (formateos y '--')
    mapping = build_mapping(datos)

    # 7) Expandir items si hay
    if isinstance(items, list) and items:
        expandir_repeat_items(doc, items)

    # 8) Canonizar + reemplazar
    ...
    if m:
        ...
        try:
            etapa_aaaa = str(
                parse_fecha_es(str(datos.get("fecha_inicio", "")).strip()).year
            )
        except Exception:
            etapa_aaaa = "XXXX"
    ...
    try:
        ...
        comment_author = (
            __author__
            if "__author__" in globals()
            else "ChatGPT (GPT-5 Thinking) + Usuario"
        )
        version_str = __version__ if "__version__" in globals() else "1.0.0"
        cp.comments = (
            f"Generado con script de Pa con soporte de {comment_author}, v{version_str}."
        )
        if not cp.title:
            cp.title = "Contrato"
        if not cp.subject:
...
def main():
    ap = argparse.ArgumentParser(
        description="Generador de contratos .docx desde plantilla con marcadores."
    )
    ap.add_argument(
        "--datos",
        required=False,
        help="Ruta a datos (.json, .csv, .xlsx) con encabezados = marcadores.",
    )
    ap.add_argument(
        "--plantilla",
        required=False,
        help="Ruta a plantilla .docx (admite [], {{}}, <<>>).",
    )
    ap.add_argument(
        "--salida",
        required=False,
    ...
    if args.about:
        ...
        print(
            "Créditos: desarrollado colaborativamente en 2025 con soporte de ChatGPT (GPT-5 Thinking)."
        )
        return

    # Solo listar marcadores
    ...
    if not (args.datos and args.plantilla and args.salida):
        raise SystemExit(
            "Debes indicar --datos, --plantilla y --salida "
            "(o usa --listar-marcadores / --about)."
        )

    out = generar_contrato(
        args.datos,
        args.plantilla,
        args.salida,
